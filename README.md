...
avocado 是纯 Python 实现的 Figma D2C（Design-to-Code）工具：输入 Figma 节点 URL，输出可直接交付的 React JSX 或 HTML + CSS。既给前端开发者日常出码，也给 AI agent 程序化调用。
...
```bash
cd packages/avocado && pip install -e ".[dev]"
```
