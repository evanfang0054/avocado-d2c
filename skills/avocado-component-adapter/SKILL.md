...
**增强字段（trace-adapter 后续迭代）**：
- `unmatched_details[].path`（祖先名链，≤8 段）+ `suggestion`（仅 2 个 skipped_by 场景的确定性 YAML 骨架，component/package 为 `<fill>`）
- `preset_matches.applied_details`：`variant_prop_hits` / `variant_prop_misses` / `leaf_dropped`（应用侧细节）
- `extractor_outputs[].error`：`not_registered` / `exception`（看 `error_detail`）/ `empty_result`（合法降级）
- `matched_by` 枚举含 `plugin`（插件标记节点，口径对齐 `recognition.recognized`）
- `data.trace.issues`：四类 top-10 排序（先修什么）
